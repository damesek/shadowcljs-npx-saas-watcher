shadow$provide.module$node_modules$vega_force$build$vega_force=function(global$jscomp$0,process,require,module,exports$jscomp$0,shadow$shims){(function(global,factory){"object"===typeof exports$jscomp$0&&"undefined"!==typeof module?factory(exports$jscomp$0,require("module$node_modules$vega_dataflow$build$vega_dataflow"),require("module$node_modules$vega_util$build$vega_util"),require("module$node_modules$d3_force$dist$d3_force")):"function"===typeof define&&define.amd?define(["exports","vega-dataflow",
"vega-util","d3-force"],factory):(global=global||self,factory((global.vega=global.vega||{},global.vega.transforms={}),global.vega,global.vega,global.d3))})(this,function(exports,vegaDataflow,vegaUtil,d3Force){function Force(params){vegaDataflow.Transform.call(this,null,params)}function rerun(df,op){return function(){df.touch(op).run()}}function simulation(nodes,_){nodes=d3Force.forceSimulation(nodes);var stopped=!1,stop=nodes.stop,restart=nodes.restart;nodes.stopped=function(){return stopped};nodes.restart=
function(){stopped=!1;return restart()};nodes.stop=function(){stopped=!0;return stop()};return setup(nodes,_,!0).on("end",function(){stopped=!0})}function setup(sim,_$jscomp$0,init,pulse$jscomp$0){var f=vegaUtil.array(_$jscomp$0.forces),n;var i=0;for(n=ForceParams.length;i<n;++i){var p=ForceParams[i];if("forces"!==p&&_$jscomp$0.modified(p))sim[p](_$jscomp$0[p])}i=0;for(n=f.length;i<n;++i){var name="forces"+i;if(init||_$jscomp$0.modified("forces",i)){p=void 0;var _=f[i];vegaUtil.hasOwnProperty(ForceMap,
...
params:[{name:"strength",type:"number","default":.1},{name:"x",type:"field"}]},{key:{force:"y"},params:[{name:"strength",type:"number","default":.1},{name:"y",type:"field"}]}]},{name:"as",type:"string",array:!0,modify:!1,"default":ForceOutput}]};var prototype=vegaUtil.inherits(Force,vegaDataflow.Transform);prototype.transform=function(_,pulse){var sim=this.value,change=pulse.changed(pulse.ADD_REM),params=_.modified(ForceParams),iters=_.iterations||300;sim?(change&&(pulse.modifies("index"),sim.nodes(pulse.source)),
(params||pulse.changed(pulse.MOD))&&setup(sim,_,0,pulse)):(this.value=sim=simulation(pulse.source,_),sim.on("tick",rerun(pulse.dataflow,this)),_.static||(change=!0,sim.tick()),pulse.modifies("index"));if(params||change||_.modified(ForceConfig)||pulse.changed()&&_.restart)if(sim.alpha(Math.max(sim.alpha(),_.alpha||1)).alphaDecay(1-Math.pow(sim.alphaMin(),1/iters)),_.static)for(sim.stop();0<=--iters;)sim.tick();else if(sim.stopped()&&sim.restart(),!change)return pulse.StopPropagation;return this.finish(_,
pulse)};prototype.finish=function(_,pulse){for(var dataflow=pulse.dataflow,args=this._argops,j=0,m=args.length,arg;j<m;++j)if(arg=args[j],"forces"===arg.name&&"link"===arg.op._argval.force){arg=arg.op._argops;for(var i=0,n=arg.length,op;i<n;++i)if("links"===arg[i].name&&(op=arg[i].op.source)){dataflow.pulse(op,dataflow.changeset().reflow());break}}return pulse.reflow(_.modified()).modifies(ForceOutput)};exports.force=Force;Object.defineProperty(exports,"__esModule",{value:!0})})}
