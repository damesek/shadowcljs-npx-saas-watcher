...
i){i||(x0=y0=0);i=2;var n=input.length,output=Array(n),x1=Math.round((input[0]-dx)/kx),y1=Math.round((input[1]-dy)/ky);output[0]=x1-x0;x0=x1;output[1]=y1-y0;for(y0=y1;i<n;)output[i]=input[i],++i;return output}}exports.bbox=bbox;exports.feature=function(topology,o$jscomp$0){return"GeometryCollection"===o$jscomp$0.type?{type:"FeatureCollection",features:o$jscomp$0.geometries.map(function(o){return feature$1(topology,o)})}:feature$1(topology,o$jscomp$0)};exports.merge=function(topology){return object$jscomp$0(topology,
mergeArcs.apply(this,arguments))};exports.mergeArcs=mergeArcs;exports.mesh=function(topology){return object$jscomp$0(topology,meshArcs.apply(this,arguments))};exports.meshArcs=meshArcs;exports.neighbors=function(objects){function line(arcs,i){arcs.forEach(function(a){0>a&&(a=~a);var o=indexesByArc[a];o?o.push(i):indexesByArc[a]=[i]})}function polygon(arcs,i){arcs.forEach(function(arc){line(arc,i)})}function geometry(o$jscomp$0,i){if("GeometryCollection"===o$jscomp$0.type)o$jscomp$0.geometries.forEach(function(o){geometry(o,
i)});else if(o$jscomp$0.type in geometryType)geometryType[o$jscomp$0.type](o$jscomp$0.arcs,i)}var indexesByArc={},neighbors=objects.map(function(){return[]}),geometryType={LineString:line,MultiLineString:polygon,Polygon:polygon,MultiPolygon:function(arcs,i){arcs.forEach(function(arc){polygon(arc,i)})}};objects.forEach(geometry);for(var i$jscomp$0 in indexesByArc){objects=indexesByArc[i$jscomp$0];for(var m=objects.length,j=0;j<m;++j)for(var k=j+1;k<m;++k){var ij=objects[j],ik=objects[k],n;(n=neighbors[ij])[i$jscomp$0=
bisect(n,ik)]!==ik&&n.splice(i$jscomp$0,0,ik);(n=neighbors[ik])[i$jscomp$0=bisect(n,ij)]!==ij&&n.splice(i$jscomp$0,0,ij)}}return neighbors};exports.quantize=function(topology,transform){function quantizePoint(point){return t(point)}function quantizeGeometry(input){switch(input.type){case "GeometryCollection":var output={type:"GeometryCollection",geometries:input.geometries.map(quantizeGeometry)};break;case "Point":output={type:"Point",coordinates:t(input.coordinates)};break;case "MultiPoint":output=
{type:"MultiPoint",coordinates:input.coordinates.map(quantizePoint)};break;default:return input}null!=input.id&&(output.id=input.id);null!=input.bbox&&(output.bbox=input.bbox);null!=input.properties&&(output.properties=input.properties);return output}if(topology.transform)throw Error("already quantized");if(transform&&transform.scale)var box=topology.bbox;else{if(!(2<=(n$jscomp$0=Math.floor(transform))))throw Error("n must be ≥2");box=topology.bbox||bbox(topology);transform=box[0];var y0=box[1],x1=
box[2],y1=box[3],n$jscomp$0;transform={scale:[x1-transform?(x1-transform)/(n$jscomp$0-1):1,y1-y0?(y1-y0)/(n$jscomp$0-1):1],translate:[transform,y0]}}var t=untransform(transform),key;y0=topology.objects;x1={};for(key in y0)x1[key]=quantizeGeometry(y0[key]);return{type:"Topology",bbox:box,transform:transform,objects:x1,arcs:topology.arcs.map(function(input){var i=0,j=1,n=input.length,p,output=Array(n);for(output[0]=t(input[0],0);++i<n;)if((p=t(input[i],i))[0]||p[1])output[j++]=p;1===j&&(output[j++]=
[0,0]);output.length=j;return output})}};exports.transform=transform$jscomp$0;exports.untransform=untransform;Object.defineProperty(exports,"__esModule",{value:!0})})}
